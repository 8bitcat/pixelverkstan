...
await page.waitForFunction(() => PV.build.finale.run?.done, null, { timeout: 10000 }); await page.waitForTimeout(300);
const verdict = await page.evaluate(() => PV.build.msg?.html || '');
ok(/deliver/.test(verdict), `kundens omdöme: ${verdict.replace(/<[^>]+>/g, '').slice(0, 80)}`);
await page.screenshot({ path: OUT + 'rest-7-omdome.png' });
const before = await page.evaluate(() => PV.game.money);
await page.evaluate(() => PV.build.guideAction('deliver'));
...
ok(after.phase === 'ready' && after.payout > 0 && /nöjd/i.test(after.modal || ''), `kunden hämtar och betalar ${after.payout} kr vid luckan: ${JSON.stringify(after)}`);
