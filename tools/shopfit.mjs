...
await page.click(`[data-opt="${optId}"]`); await page.waitForTimeout(400);
...
await page.click('[data-item="lokal2"]'); await page.waitForTimeout(300);
...
ok(f2.lokal === 2 && f2.open === 6 && f2.stats.drag >= 6 && f2.stats.trivsel >= 6, `renoverat och inrett (${JSON.stringify(f2)})`);
...
// Datorhuset
await page.click('[data-h="fit"]'); await page.waitForSelector('[data-tab="lager"]'); await page.click('[data-tab="lager"]'); await page.waitForSelector('[data-item="lokal3"]');
await page.click('[data-item="lokal3"]'); await page.waitForTimeout(300); await page.click('[data-close]'); await page.waitForTimeout(800);
const f3 = await page.evaluate(() => ({ lokal: PV.floor.lokal, open: PV.floor.openSlots }));
ok(f3.lokal === 3 && f3.open === 8, `Datorhuset (${JSON.stringify(f3)})`);
...
ok(f4.lokal === 3 && f4.slot0 === 'brand' && f4.items >= 8, `sparat och laddat (${JSON.stringify(f4)})`);
