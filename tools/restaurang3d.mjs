...
const buildAll = () => page.evaluate(async () => {
  ...
  for (let guard = 0; guard < 60; guard++) {
    ...
    if (s.kind === 'act') { const a = L.ACTION[s.id]; const done = L.actSet(v.b, a.id); const i = a.points.findIndex((p, k) => !done.has(k)); v.doAction(a, i); v.b.time += 8; /* tillagningen tar tid */ log.push('act:' + a.id); }
