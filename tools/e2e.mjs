...
const [prefix = "e", W = "1400", H = "860"] = process.argv.slice(2);
...
await page.mouse.click(cpos.x, cpos.y); await page.waitForTimeout(400);
await page.screenshot({ path: OUT + prefix + "a-order.png" });
await page.click('button:has-text("Ta emot")'); await page.waitForTimeout(500);
...
await deliver.click(); await page.waitForTimeout(500);
await page.screenshot({ path: OUT + prefix + '5-result.png' });
...
await page.screenshot({ path: OUT + prefix + '6-paid.png' });
console.log('efter:', JSON.stringify(await page.evaluate(() => ({ money: PV.game.money, xp: PV.game.xp, step: PV.game.tutorialStep, served: PV.game.stats.served }))));
console.log(errors.join(String.fromCharCode(10)) || "inga fel");
await browser.close();
