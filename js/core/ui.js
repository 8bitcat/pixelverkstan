...
export function showLevelUp(game, info) {
  ...
  const ga = game.gala ? game.gala(y) : null;
  const gala = ga ? `<div class="gala"><b>🏆 ${y >= game.lastYear ? 'Slutgalan' : `Galan ${y}`}: ${esc(ga.title)}</b><div class="medals">${ga.medals.map((m) => `<div><span class="md">${m.medal}</span><b>${m.icon} ${esc(m.name)}</b><small>${esc(m.text)}</small></div>`).join('')}</div>${ga.gold ? `<small>${ga.gold} guld → ryktet växer.</small>` : ''}</div>` : '';
  const body = `<p style="font-size:21px;margin-top:0">Det har blivit <b>${y}</b>${info.era?.title ? ` – ${esc(info.era.title)}` : ''}! Grossisten har <b>${fresh.length}</b> nya delar${gone ? ` och ${gone} gamla har slutat säljas` : ''}.</p>${gala}${bokslut}${news}
